...
//static void
pub fn d_convertversionnumbers() {
	let major: i32;
	let minor: i32;
...
pub fn d_home() -> std::string::String {
	let mut userhome: std::string::String = std::string::String::new();

	let key = "HOME";
	match std::env::var(key) {
    	Ok(val) => {
    		userhome = val.as_str().to_string();
    	}
    	Err(e) => println!("{} is not defined in the environment; {}", key, e)
	};
	userhome
}
