...
const FONTFILE: &'static str = "srb2.ttf";
const FONTPOINTSIZE: u32 = 12;

// Default font foreground colours
const DEFAULTFONTFGR: u32 = 255;
const DEFAULTFONTFGG: u32 = 255;
const DEFAULTFONTFGB: u32 = 255;
const DEFAULTFONTFGA: u32 = 255;

// Default font background colours
const DEFAULTFONTBGR: u32 = 0;
const DEFAULTFONTBGG: u32 = 0;
const DEFAULTFONTBGB: u32 = 0;
const DEFAULTFONTBGA: u32 = 255;

/*
#ifndef SDL_TTF_COMPILEDVERSION
const SDL_TTF_COMPILEDVERSION \
	SDL_VERSIONNUM(TTF_MAJOR_VERSION, TTF_MINOR_VERSION, TTF_PATCHLEVEL)
#endif

#ifndef SDL_TTF_VERSION_ATLEAST
const SDL_TTF_VERSION_ATLEAST(X, Y, Z) \
...
fn main() {
	...
	// TODO: argc will most likely not be used
	let argc: usize = argv.len() - 1; // the first string is not an argument!

	// this initializes all global variables via utilizing structs. 
	//initialize_all_variables();

	// static variable within m_argv.rs
	let mut found: usize = 0;

	if m_argv::m_check_parm(cmd_args::NOLOG, &argv, found) == 0 {
		init_logging(&argv, found);
	}
	i_system::i_startup_system();

	// Initialize SRB2 things
	//CONS_Printf("Setting up SRB2...\n");
	//D_SRB2Main();
...
pub fn init_logging(argv: &Vec<String>, found: usize) {
	let logdir = d_main::d_home();
	println!("{}", logdir);

	/*let my_time: time_t;
	let timeinfo: tm;*/
	let format: std::string::String;
	let reldir: std::string::String;
	...
	if m_argv::m_check_parm(cmd_args::LOGFILE, &argv, found) == m_argv::m_is_next_parm(&argv, found) {
		format = m_argv::m_get_next_parm(&argv, found);
		fileabs = m_misc::m_is_path_absolute(format);
	} else {
		format = "log-%Y-%m-%d_%H-%M-%S.txt".to_string();
		fileabs = false;
	}

	if fileabs {
		//strftime(logfilename, sizeof logfilename, foprmat, timeinfo);
	} else {
		if m_argv::m_check_parm(cmd_args::LOGDIR, &argv, found) == m_argv::m_is_next_parm(&argv, found) {
			reldir = m_argv::m_get_next_parm(&argv, found);
		} else {
			reldir = std::string::String::from("logs");
		}

		if m_misc::m_is_path_absolute(reldir.to_string()) {
			//left = snprintf(logfilename, sizeof logfilename, "%s"PATHSEP DEFAULTDIR PATHSEP"%s"PATHSEP, logdir, reldir);
		}  else {
			//left = snprintf(logfilename, sizeof logfilename, "."PATHSEP"%s"PATHSEP, reldir);
		}

		//strftime(&logfilename[left], sizeof logfilename - left, format, timeinfo);
	}
	
	//M_MkdirEachUntil(logfilename, m_path_parts(logdir) - 1,
	//								m_path_parts(logfilename) - 1, 0755);

	println!("Logging is turned on.");
}